...
func updateUI():
	...
	$texGun.modulate = Color(0.1,0.1,0.1)+Color(0.9,0.9,0.9)*int(Globals.getUnlocked(Globals.getGunName()))
	$btnUnlock.visible = not Globals.getUnlocked(Globals.getGunName())
	$btnUnlock.text = str(Globals.getUnlockCostGun()) + "$"
